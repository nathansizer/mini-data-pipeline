...
From here, the next step is to integrate all of the above with an AWS EC2 instance. By running everything natively in the cloud, we can access our data whenever, and wherever we want, but also inclue scheduling and automation as we can ensure the VM instance will always be running when we need it.
